...
PGraphics pg; 
String[] fileString; //string to hold the txt file
...
void draw() {
  ...
  for ( int i = 0; i < fileString.length; i++) {
    ...
    fill(0);
    textSize(16);
    text(pair[0], 0,  i*step);
    text(pair[1], 200, i*step);
  }
  translate(400, -30);
  for ( int i = 0; i < fileString.length; i++) {
    String[] pair = split(fileString[i], '\t');
      ...
      pushMatrix();
       scale(0.15);
       translate(0, i*(step+200));
        radical.render(this.g);
        popMatrix();
     }
  }
     popMatrix();
...
cp5.getController("enter encoding").hide();
 dropLayers.hide();
     ...
     cp5.getGroup("g1").hide();
      cp5.getController("clear").hide();
       cp5.getController("quit").hide();
       cp5.getController("Encode").hide();
       cp5.getController("Decode").hide();
       drop0.hide();
  }
  void Encode() {
    encodingState = true;
