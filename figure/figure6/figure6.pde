...
TShape s,u,q,p;
int nRadicals = 17; //total number of radicals draw types used to make the symbols
int characters = 4; //number of symbols
...
void initiateFigures() {
  ...
  //figures far away...
  for (int i = 0; i < n; i++) {
    PVector p = new PVector (random(width), random(h/2, height));
...
void generateSymbols() {
  for ( int i = 0; i < characters; i++) { 
    ...
    roots.get(i).setPosition(new PVector (random(width), random(h*3)));
    float factor = random(5.0, 5.1); 
