...
int characters = 100; //number of symbols
int n = 40; //number of figures
...
void generateGrids() {
  //TColor col = ColorRange.FRESH.getColor();
...
void generateFigures() {
  for (int i = 0; i < n; i++) {
    PVector p = new PVector (random(2), random(2));
    figures[i].setlocation(p); 
    float f = random(0.8, 1.5);
